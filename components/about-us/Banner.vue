<style scoped>
* {
  font-family: 'Inter', sans-serif;
}
body {
  @apply text-body
}
</style>

<template>
  <section class="relative">
    <img src="~/assets/images/about-us/about-us-banner.png" alt="About Us Banner" class="w-full relative hidden sm:block">
    <img src="~/assets/images/about-us/about-us-banner-mobile.png" alt="About Us Banner" class="w-full relative sm:hidden">

    <div class="custom-container absolute bottom-4 md:bottom-12 lg:bottom-16 left-0">
      <div class="bg-white w-full md:w-fit px-4 md:pl-10 md:pr-[100px] py-5 md:py-6">
        <h1 class="text-dark text-2xl lg:text-[45px] lg:leading-[60px] font-semibold">
          <span class="text-[#8D8D8F]">Committed to revolutionize</span> <br>
          <span>the transportation landscape</span>
        </h1>
      </div>
    </div>
  </section>
</template>